...
function PostThree ( props ) {
    const { post, adClass = 'mb-4', isLazy = false, isOriginal = false, btnText = "Read more", btnAdClass = 'btn-link btn-underline', isCalender = false } = props;
    ...
    return (
        <div className={ `post post-list ${ post.type === 'video' ? 'post-video' : '' } ${ adClass }` }>
                        ...
                        }
                        {
                            isCalender ?
                                <div className="post-calendar">
                                    <span className="post-day">{ ( new Date( post.date ).getDay() + 1 ).toLocaleString().padStart( 2, 0 ) }</span>
            ...
            <div className="post-details">
                ...
                <ALink href={ `/blog/single/${ post.slug }` } className={ `btn btn-primary ${ btnAdClass }` }>{ btnText }<i className="d-icon-arrow-right"></i></ALink>
            </div>
        </div >
    )
}
